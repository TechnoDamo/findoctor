...
async def find_goal(conn: AsyncConnection, goal_id: str) -> dict | None:
    """Получение цели по id."""
    return await conn.fetchrow(_queries["find_goal"], {"goal_id": goal_id})
...
async def update_goal(conn: AsyncConnection, goal_id: str, data: dict) -> dict:
    """Обновление цели."""
    return await conn.fetchrow(
        _queries["update_goal"], {"goal_id": goal_id, **data}
    )


async def delete_goal(conn: AsyncConnection, goal_id: str) -> None:
    """Удаление цели."""
    await conn.execute(_queries["delete_goal"], {"goal_id": goal_id})
