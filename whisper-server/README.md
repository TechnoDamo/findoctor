...
- [Интеграция с ПрофИИт](#интеграция-с-профиит)
...
После запуска прокси слушает на `127.0.0.1:8080`. Эндпоинт — `POST /chat/completions` (полная совместимость с форматом Chat Completions API, который использует бэкенд ПрофИИт).
...
```bash
# Формат, идентичный тому, что отправляет бэкенд ПрофИИт
...
## Интеграция с ПрофИИт

whisper-server — полностью локальная замена облачным STT-провайдерам для голосового режима ПрофИИт.
