...
AI-чат ПрофИИта — ассистент с поддержкой текста и голоса. Пользователь отправляет текстовое или аудио-сообщение, backend вызывает STT/LLM/TTS-провайдеров и возвращает ответ. Дополнительно может включаться поток рекомендаций с RAG и поиском.
...
`llm_text.txt`:
```
Ты — финансовый ассистент ПрофИИта.
У тебя мужской пол.
...
`llm_voice.txt`:
```
Ты — голосовой финансовый ассистент ПрофИИт. Твой пол — мужской.
