...
SELECT %(transaction_id)s, %(tag_id)s
WHERE EXISTS (
  ...
  AND EXISTS (
    SELECT 1 FROM tags
    WHERE id = %(tag_id)s AND user_id = %(user_id)s
)
...
DELETE FROM transaction_tags tt
USING transactions t, tags tg
WHERE tt.transaction_id = %(transaction_id)s
  AND tt.tag_id = %(tag_id)s
  AND tt.transaction_id = t.id
  AND tt.tag_id = tg.id
  AND t.user_id = %(user_id)s
  AND tg.user_id = %(user_id)s;
